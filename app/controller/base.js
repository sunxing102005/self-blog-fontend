...
module.exports = class extends think.Controller {
  ...
  getRecent() {
    ...
    return _asyncToGenerator(function* () {
      const content = yield _this2.model('content').where({ status: 99, type: 'post', category_id: ['IN', [1, 2]] }).cache('recent_content').limit(5).order('create_time desc').fieldReverse('content,markdown').select();

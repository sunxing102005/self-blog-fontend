...
module.exports = class extends Base {
  ...
  listNoPageAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const recommend = _this2.get('recommend');
      let orderby = _this2.get('orderby');
      let fieldReverse = _this2.get('fieldReverse');
      const category_id = _this2.get('category_id');
  ...
  getLastAndNextBlogAction() {
    ...
    return _asyncToGenerator(function* () {
      const date = _this7.post('date'); // 当前博客
      const lastBlogSql = ` 
      ...
      const nextBlogSql = ` 
        ...
        and publish_time>'${date}' and type = 'post')
        `;
      const lastBlog = yield _this7.model().query(lastBlogSql);
      const nextBlog = yield _this7.model().query(nextBlogSql);
